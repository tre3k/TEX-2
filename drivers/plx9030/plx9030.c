...
static ssize_t device_file_read(struct file *f, char __user *buff, size_t count, loff_t *offset){
...
#endif
		iobuff = (unsigned char) ioread8(mydata->cs3_mem+mydata->offset);
#ifdef DEBUG_MODE 
		printk(KERN_INFO MODULE_NAME ": read data: 0x%.2x\n",iobuff & 0xff);
#endif
		copy_to_user(buff,(const void *)&iobuff,1);
	...
	case CS2_SET_ADDR16:
#ifdef DEBUG_MODE  
		printk(KERN_INFO MODULE_NAME ": SET CS2 for 16bit, offset: %d\n",mydata->offset);
#endif
		iobuff16 = (unsigned short int) ioread16(mydata->cs2_mem+mydata->offset*2);
#ifdef DEBUG_MODE 
		printk(KERN_INFO MODULE_NAME ": read data: 0x%.4x\n",iobuff16 & 0xffff);
#endif
	...
	case CS3_SET_ADDR16:
#ifdef DEBUG_MODE  
		printk(KERN_INFO MODULE_NAME ": SET CS3 for 16bit, offset: %d\n",mydata->offset);
#endif
		iobuff16 = (unsigned short int) ioread16(mydata->cs3_mem+mydata->offset*2);
#ifdef DEBUG_MODE 
		printk(KERN_INFO MODULE_NAME ": read data: 0x%.4x\n",iobuff16 & 0xffff);
#endif
		copy_to_user(buff,(const void *)&iobuff16,2);
...
#ifdef DEBUG_MODE  
		printk(KERN_INFO MODULE_NAME ": SET CS2 for 16 bit, write data: 0x%.4x\n",iobuff16);
#endif
		iowrite16(iobuff16,mydata->cs2_mem+mydata->offset*2);
		break;

	case CS3_SET_ADDR16:
		copy_from_user(&iobuff16,buff,2);
#ifdef DEBUG_MODE  
		printk(KERN_INFO MODULE_NAME ": SET CS3 for 16 bit, write data: 0x%.4x\n",iobuff16);
#endif
